...
public class RtpPacket implements Runnable {

	public static final String TAG = "RtpPacket";
	 ...
	 * @throws IOException
	 */
	public RtpPacket(Channel channel, InetSocketAddress destination) {
	...
	/**
	 * Returns an approximation of the bitrate of the RTP stream in bits per
	 * second.
	 */
			...
			while (mBufferCommitted.tryAcquire(4, TimeUnit.SECONDS)) {
				if (mOldTimestamp != 0) {
					...
					if ((mTimestamps[mBufferOut] - mOldTimestamp) > 0) {
						...
						// Log.d(TAG,"delay: "+d+" d:
						// "+(mTimestamps[mBufferOut]-mOldTimestamp)/1000000);
					...
					if (delta > 500000000 || delta < 0) {
						// Log.d(TAG,"permits:
						// "+mBufferCommitted.availablePermits());
						delta = 0;
					}
				}
				// mReport.update(mPackets[mBufferOut].getLength(),
				// (mTimestamps[mBufferOut] / 100L) * (mClock / 1000L) /
				// 10000L);
				mOldTimestamp = mTimestamps[mBufferOut];
				if (mCount++ > 30) {
					if (mTransport == TRANSPORT_UDP) {
						ByteBuf buf = Unpooled.copiedBuffer(mPackets[mBufferOut].getData());
						io.netty.channel.socket.DatagramPacket dp = new io.netty.channel.socket.DatagramPacket(buf, destination);
	...
	protected static class Statistics {
		...
		public void push(long value) {
			...
			if (c < 40) {
				// We ignore the first 40 measured values because they may not
				// be accurate
				c++;
				m = value;
			} else {
